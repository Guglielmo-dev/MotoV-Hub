...
import { Bike, ArrowRight, Lock, CheckCircle2, Loader2, AlertCircle } from "lucide-react";
...
export function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      toast({
        title: "Errore",
        description: "Token mancante nel link",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Errore",
        description: error.message || "Token scaduto o non valido",
        variant: "destructive"
      });
    } finally {
      setIsPending(false);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="max-w-md w-full bg-card border border-white/10 rounded-2xl p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6">Il link di recupero non contiene un token valido o è incompleto.</p>
          <Link href="/forgot-password">
            <button className="w-full py-3 px-4 bg-primary text-black font-bold rounded-xl">Richiedi nuovo link</button>
          </Link>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex bg-background">
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-16 xl:px-24 z-10">
        <div className="max-w-md w-full mx-auto">
          ...
          {!isSuccess ? (
            <>
              <h2 className="text-3xl font-bold mb-2">Nuova Password</h2>
              <p className="text-muted-foreground mb-8">
                Inserisci la tua nuova password per accedere al tuo garage.
              </p>
            ...
            <div className="text-center py-8 animate-in zoom-in-95 duration-500">
              ...
              <h2 className="text-3xl font-bold mb-4 uppercase tracking-tighter italic">Motore Acceso!</h2>
              <p className="text-muted-foreground mb-8">
                La tua password è stata aggiornata. Ora puoi tornare in sella al tuo account.
              </p>
              <Link href="/login">
                <button className="w-full py-4 px-4 bg-primary text-black font-black uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-95">
                  Vai al Login
                </button>
              </Link>
            </div>
      ...
      <div className="hidden lg:block lg:w-1/2 relative border-l border-white/5">
        ...
        <img
          ...
          alt="Motorcycle night landscape"
